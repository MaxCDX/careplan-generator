...
A production-oriented learning project for building a specialty-pharmacy care plan generation workflow.

This project is being developed incrementally. The current version is a small synchronous MVP that proves the first end-to-end flow:

```text
Frontend form -> FastAPI backend -> OpenAI -> generated care plan -> frontend display
```
...
- Day 1: requirements analysis and system design
- Day 2: minimal synchronous MVP
  - FastAPI backend
  - Next.js frontend
  - OpenAI integration
...
Because storage is currently in memory only, generated records disappear when the backend restarts.
...
- strict healthcare validation
- duplicate detection
- warning/error business rules
- Redis/Celery background jobs
- WebSocket or polling job status
- authentication
- PDF/file upload
- monitoring
- cloud deployment

These will be added step by step after the synchronous MVP pain points are clear.
...
This repo intentionally starts simple. The first version is synchronous, in-memory, and minimally structured so the limitations are visible before adding database persistence, background workers, validation layers, and production infrastructure.
# Care Plan Generator
